  ...
  const handleShare = async (method) => {
    ...
    switch (method?.type) {
      ...
      case 'whatsapp':
        ...
        window.open(`https://wa.me/?text=${encodeURIComponent(`Check out this amazing offer: ${couponData?.shareUrl}`)}`);
        break;
      default:
        break;
    }
              ...
              <div className="bg-white rounded-xl border border-border shadow-level-1 p-6">
                ...
                <div className="flex items-center space-x-4 p-4 bg-muted rounded-lg">
                  <div className="w-16 h-16 bg-primary rounded-lg flex items-center justify-center overflow-hidden">
                    <img 
                      src={couponData?.thumbnail} 
                      alt="Coupon preview"
                      className="w-full h-full object-cover"
                    />
                  </div>
                  <div className="flex-1">
                    <h4 className="font-medium text-foreground">{couponData?.title}</h4>
                    <p className="text-sm text-muted-foreground">{couponData?.businessName}</p>
                    <div className="flex items-center space-x-4 mt-2 text-xs text-muted-foreground">
                      <span>Expires: {new Date(couponData.expiryDate)?.toLocaleDateString()}</span>
                      <span>•</span>
                      <span>Limit: {couponData?.usageLimit} uses</span>
                    </div>
                  </div>
                  <Button
                    variant="outline"
