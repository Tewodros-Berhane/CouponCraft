...
const ensureUploadsEnabled = (res) => {
  if (config.uploadsEnabled) return true;
  res.status(503).json({ message: 'Uploads are disabled in this environment' });
  return false;
};
...
const sign = (payload) =>
  crypto.createHmac('sha256', config.jwtSecret).update(payload).digest('hex');
...
uploadsRouter.put(
  ...
  asyncHandler(async (req, res) => {
    if (!ensureUploadsEnabled(res)) return;
    const { key } = req.params;
